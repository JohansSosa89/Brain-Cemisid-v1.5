...
__global__ void correct(unsigned int * d_vectorFlags, unsigned char * d_ptr, unsigned char *d_desiredOutput);

__global__ void reset(unsigned int * d_vectorFlags, unsigned char *d_ptr);

__global__ void recognize(unsigned char * d_vectorNeuron, unsigned int *d_vectorFlags,
...
extern "C"
stateNeuralNetwork recognize(NeuralNetwork * neuralSenses, const SizeNet & sizeNet, unsigned char * h_pattern, Interface * interface, Statistic *& statistic){
    int *d_countHit;
    unsigned char *d_arrayCategory, *d_idsNeuron;
    unsigned char *d_vectorNeuron, *d_pattern,*d_ptr;
    unsigned int *d_vectorFlags;
...
void correct(NeuralNetwork * neuralSenses, unsigned char desiredOutput, int maxThreadsPerBlock, Statistic *&statistic){
    ...
    GpuTimer timer;
    // It allocates memory on the device
    checkCudaErrors(cudaMalloc(&d_vectorFlags,sizeof(unsigned int) * SIZE_FLAGS* (*neuralSenses->ptr)));
    checkCudaErrors(cudaMalloc(&d_desiredOutput,sizeof(unsigned char)));
    checkCudaErrors(cudaMalloc(&d_ptr,sizeof(unsigned char)));
    // copy from host to device
    checkCudaErrors(cudaMemcpy(d_vectorFlags, neuralSenses->vectorFlags, sizeof(unsigned int) * SIZE_FLAGS * (*neuralSenses->ptr), cudaMemcpyHostToDevice));
    checkCudaErrors(cudaMemcpy(d_ptr, neuralSenses->ptr, sizeof(unsigned char), cudaMemcpyHostToDevice));
    ...
    // copy from device to host
    checkCudaErrors( cudaMemcpy( neuralSenses->vectorFlags, d_vectorFlags, sizeof(unsigned int) * SIZE_FLAGS *(*neuralSenses->ptr), cudaMemcpyDeviceToHost));
    // Free memory on device Reserved
    checkCudaErrors(cudaFree(d_vectorFlags));
    checkCudaErrors(cudaFree(d_ptr));
...
extern "C"
void safeRelation(NeuralNetwork *neuralSenses, const SizeNet &sizeNet, RelationNetwork relationSenses, Statistic *& statistic, Interface *interface, stateNeuralNetwork *stateSense, unsigned int *whisCategory){
    int * d_countHit;
    stateNeuralNetwork *d_stateSense;
    unsigned char *d_idsNeuron;
    unsigned int *d_vectorFlags, *d_whisCategory;
    ...
    // copy from host to device
    checkCudaErrors(cudaMemcpy(d_vectorFlags, neuralSenses->vectorFlags, sizeNet.sizevectorFlags, cudaMemcpyHostToDevice));
    checkCudaErrors(cudaMemcpy(d_whisCategory, whisCategory, sizeof(unsigned int), cudaMemcpyHostToDevice));
...
void newItemCulturalNet(CulturalNet * addNet, int protocol, int LPA, int LPT ){
    ...
    //call kernel saveRelation
    timer.Start();
    newItemCulturalNet<<<gridSize, blockSize>>>(*d_data, *d_valve, trueKNW, d_protocol, d_LPA, d_LPT, d_newData);
    timer.Stop();
    checkCudaErrors(cudaDeviceSynchronize());
    checkCudaErrors(cudaGetLastError());
    calculateStatistic(timer.Elapsed(),statistic,SAFERELATION);
...
//cuda kernel
__global__ void safeRelation(unsigned int *d_vectorFlags, int *d_countHit, unsigned char *d_idsNeuron, unsigned char *d_vectorEar,
                             unsigned char *d_vectorSigth, stateNeuralNetwork *stateSense, unsigned int *d_whisCategory){
...
__global__ void recognize(unsigned char * d_vectorNeuron, unsigned int *d_vectorFlags, unsigned char *d_pattern, int *d_countHit, unsigned char *d_ptr,
                          unsigned char *d_arrayCategory, unsigned char *d_idsNeuron, Lock lock){
    __shared__ unsigned char sharedVectorNeuron[SIZE_CHARACTERISTIC];
    __shared__ unsigned int sharedVectorFlags[SIZE_FLAGS];
...
__global__ void correct(unsigned int *d_vectorFlags, unsigned char *d_ptr, unsigned char *d_desiredOutput){
    ...
    unsigned int ratio, dis;
    unsigned char hit,category;
...
__global__ void newItemCulturalNet(unsigned char *d_data, bool *d_valve, int d_LPA, int d_LPT, unsigned char d_newData){
    int columna = 0;
    int level = 0;
    level = blockIdx.x % 5;
