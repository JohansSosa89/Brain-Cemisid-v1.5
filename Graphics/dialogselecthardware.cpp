...
DialogSelectHardware::DialogSelectHardware(QWidget *parent, int selectDevice) : QDialog(parent), ui(new Ui::DialogSelectHardware){
    deviceCount = 0;
    deviceProp = NULL;
...
void DialogSelectHardware::setListDevice(){
    ...
    QString text("Detectado(s) "+QString::number(deviceCount)+" Dispositivo(s) Compatible(s) con CUDA");
    QMessageBox information("Dispositivos Detectados",text,QMessageBox::NoIcon,QMessageBox::Ok,QMessageBox::NoButton,QMessageBox::NoButton);
    information.setWindowIcon(QPixmap("icons/cuda.png"));
    information.setWindowFlags(information.windowFlags() & ~Qt::WindowCloseButtonHint);
    information.exec();
